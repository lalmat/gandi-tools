...
module.exports = async (path) => {
  console.log('GANDI SCRAPER :: Starting browser and new tab');
  ...
  try {
    ...
    console.log('GANDI SCRAPER :: ERROR :: Woops, something goes wrong...');
    console.log(e);
  }
...
async function pageBills(page, token) {
  console.log("GANDI SCRAPER :: Accessing invoices page");
...
async function scrapBills(page) {
  console.log('GANDI SCRAPER :: Scrapping invoice data on first page only');
...
async function downloadBills(page, path, bills) {
  for (let i=0; i<bills.length; i++) {
    ...
    if (fs.existsSync(filename)) {
      console.log(`GANDI SCRAPER :: Skipped, file ${filename} exists.`);
      continue;
    }
...
async function downloadFile(page, link, filename) {
  console.log(`GANDI SCRAPER :: Downloading file ${link} into ${filename}`);
